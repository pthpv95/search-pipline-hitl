...
def _run_with_auto_approve(graph, state, config):
    """Run graph to completion, auto-approving any human review interrupts."""
...
def test_graph_loops_only_until_max_loops():
    ...
    result = _run_with_auto_approve(graph, state, config)
...
def test_human_review_auto_approved():
    ...
    state = ResearchState(topic="review topic", max_loops=1)
    result = _run_with_auto_approve(graph, state, config)
...
def test_token_usage_and_timings_are_tracked():
    ...
    state = ResearchState(topic="usage topic", max_loops=1)
    result = _run_with_auto_approve(graph, state, config)
...
def test_checkpoint_state_is_available_after_run():
    ...
    _run_with_auto_approve(graph, state, config)
